...
public class SoundSensor : SensorBase
{
    ...
    public override void Sense()
    {
        ...
        bool inRange     = UnityEngine.Vector3.Distance(_origin.position, _player.position) < _hearingRange;

        if (makingNoise && inRange)
        {
            _blackboard.Set<bool>(BB.HeardNoise, true);
            _blackboard.Set<UnityEngine.Vector3>(BB.NoisePosition, _player.position);
